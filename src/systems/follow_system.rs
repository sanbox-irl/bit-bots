use super::{scene_graph::SceneGraph, Approach, ComponentList, Follow, Name, Transform, Vec2};
    ...
    for follow_c in afs.iter_mut() {
        ...
        if let Some(target) = &follow_c.inner_mut().target.target {
            if let Some(target_position) = transforms.get(&target).map(|tc| tc.inner().world_position()) {
                ...
                let our_transform_c = transforms.get_mut_or_default(&follow_c.entity_id(), scene_graph);
